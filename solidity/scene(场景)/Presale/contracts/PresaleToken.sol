 ...
 * 预售代币合约
 */
contract PresaleToken is IERC20, Ownable {
    ...
    //基本信息
    string private _name = "Presale Token"; //名称
    string private _symbol = "presale"; //标志符
    uint8 private _decimals = 18; //小数点位数

    uint256 private _totalSupply; //已发行总额
    address public presaleMarket; //预售市场合约地址

    modifier onlyMarket() {
        require(msg.sender == presaleMarket);
        _;
    }

    /// @notice 构造函数
    /// @param _presaleMarket 预售市场合约地址
    constructor(address _presaleMarket) {
        require(address(0) != _presaleMarket);
        presaleMarket = _presaleMarket;
    }
    ...
    function mint(address account, uint256 amount) public onlyMarket {
    ...
    function setPresaleMarket(address _presaleMarket) public onlyOwner {
        presaleMarket = _presaleMarket;
    }
}
